...
def search_query_intent(query: str) -> str:
    server = _server()
    normalized = server._normalize_text(query)
    if not normalized:
        return "mixed"
    query_tokens = server._query_tokens(query)
...
def search_artist_seed_tracks(query: str, limit: int):
    server = _server()
    artists = server._assistant_tool_search_artists(query, 2)
    if not artists:
        return []
    ...
    for artist in artists:
        artist_name = server._normalize_text(artist.get("name"))
        ...
        if not artist_id:
            continue
        try:
            payload = server._build_artist_details_payload(artist_id)
        except Exception:
            payload = {}
...
def search_albums_for_artist_name(artist_name: str):
    server = _server()
    normalized_artist = trim_text(artist_name)
    if not normalized_artist:
        return {}
    direct_artists = server._assistant_tool_search_artists_direct(normalized_artist, 1)
    ...
    if not artist_id:
        return {}
    try:
        return server._build_artist_details_payload(artist_id)
    except Exception:
        return {}
...
def search_tracks_direct(query: str, limit: int):
    server = _server()
    ...
    results = server._ytmusic_song_search(query, max(limit, 12))
    if not results:
        results = server._ytdlp_song_search(query, max(limit, 12))
...
def search_artists_direct_cached(query: str, limit: int):
    server = _server()
    query = (query or "").strip()
    ...
    results = server._assistant_tool_search_artists_direct(query, limit)
    final_results = [dict(item) for item in list(results or [])[:limit] if isinstance(item, dict)]
    store_search_result("artists_direct", cache_key, final_results)
    return [dict(item) for item in final_results]


def search_albums_direct(query: str, limit: int):
    server = _server()
    query = (query or "").strip()
    limit = max(1, min(limit, 18))
    if not query:
    ...
    direct_results = server._search_upstream_call_with_retry(
    ...
    if not albums:
        fallback_results = server._search_upstream_call_with_retry(
...
def search_albums_blended(query: str, limit: int):
    server = _server()
    query = (query or "").strip()
    limit = max(1, min(limit, 18))
    if not query:
    ...
    direct_results = server._upstream_call_with_retry(
        lambda: server.ytmusic.search(query, filter="albums", limit=max(limit, 8)),
        attempts=server.UPSTREAM_RETRY_ATTEMPTS,
        backoff_seconds=server.UPSTREAM_RETRY_BACKOFF_SECONDS,
        default=[],
    ...
    track_anchor_future = server.recommendation_executor.submit(
        server._ytmusic_song_search,
        query,
        max(6, limit),
    )
    query_artist_future = server.recommendation_executor.submit(
    ...
    for index, track in enumerate(track_anchor_results[:4]):
        ...
        if not artist_name:
            continue
        anchor_artist_payload_futures[index] = server.recommendation_executor.submit(
    ...
    if len(results) < limit:
        fallback_results = server._upstream_call_with_retry(
...
def search_tracks_blended(query: str, limit: int):
    server = _server()
    query = (query or "").strip()
    ...
    direct_pool = server._ytmusic_song_search(query, max(limit, 14))
    if not direct_pool:
        direct_pool = server._ytdlp_song_search(query, max(limit, 14))
    ...
    if anchor_track is not None and anchor_track.get("id"):
        anchor_text = server._normalize_text(
        ...
        if not query_tokens or any(token in anchor_text for token in query_tokens):
            similar_tracks_future = server.recommendation_executor.submit(
                server._assistant_tool_get_similar_tracks,
                anchor_track["id"],
                max(6, min(10, limit // 2)),
            )
    artist_seed_future = server.recommendation_executor.submit(
    ...
    if len(results) < limit:
        broad_results = server._upstream_call_with_retry(
    ...
    if len(results) < limit:
        add_tracks(server._ytdlp_song_search(query, limit))
...
def semantic_search_cache_key(req, namespace: str) -> str:
    server = _server()
    payload = {
        "namespace": namespace,
        "limit": max(int(req.limit or 0), 0),
        "profile_key": server._recommendation_profile_key(req),
    }
    return hashlib.sha1(
...
def semantic_search_lexical_score(query: str, *texts) -> float:
    server = _server()
    normalized_query = server._normalize_text(query)
    tokens = server._query_tokens(query)
    normalized_texts = [
        server._normalize_text(text)
        for text in texts
        if server._normalize_text(text)
    ]
...
def semantic_search_anchor_tracks(
    ...
    *,
    ...
    server = _server()
    provided = server._recommendation_unique_snapshot_tracks(
    ...
    normalized_query = server._normalize_text(req.query)
    query_tokens = server._query_tokens(req.query)
    seen_signatures = set()
    ranked = []
    ...
    def add_anchor(raw_track, index: int, base_score: float) -> None:
        ...
        if normalized is None:
            return
        signature = server._recommendation_track_signature(normalized)
        if not signature or signature in seen_signatures:
            return
        seen_signatures.add(signature)
        title_text = server._normalize_text(normalized.get("title") or "")
        artist_text = server._normalize_text(
...
def semantic_search_anchor_artist_names(anchor_tracks, limit: int = 6):
    server = _server()
...
def semantic_search_vectors(req, profile):
    server = _server()
    query_text = server._recommendation_trim_text(req.query)
    query_vector = []
    if query_text:
        query_key = server._recommendation_text_embedding_key("semantic_search_query", query_text)
        if query_key:
    ...
    semantic_query_vector = server._vector_weighted_average(
        [
            (query_vector, 2.2),
    ...
    semantic_context_vector = server._vector_weighted_average(
        [
...
def semantic_search_vector_similarities(candidate_vector, search_vectors, profile):
    server = _server()
    vectors = profile.get("vectors") or {}
    ...
    return {
        "query": server._assistant_cosine_similarity(
            candidate_vector,
            search_vectors.get("current_query_vector") or [],
        ),
        "semantic_query": server._assistant_cosine_similarity(
            candidate_vector,
            search_vectors.get("semantic_query_vector") or [],
        ),
        "context": server._assistant_cosine_similarity(
            candidate_vector,
        ...
        "short": server._assistant_cosine_similarity(
            candidate_vector,
...
def _semantic_track_suggestion_text(track: Optional[Dict[str, Any]]) -> str:
    ...
    if not title:
        return ""
    if artist and server._normalize_text(artist) not in server._normalize_text(title):
...
def _fast_suggestion_cache_key(req) -> str:
    server = _server()
    payload = {
        "namespace": "fast_suggestions_v1",
        "query": server._normalize_text(req.query),
...
def _fast_lexical_suggestion_score(query: str, suggestion_text: str) -> float:
    server = _server()
    normalized_query = server._normalize_text(query)
    normalized_text = server._normalize_text(suggestion_text)
    if not normalized_query or not normalized_text:
        return 0.0
    if normalized_text == normalized_query:
        return 0.0
    score = semantic_search_lexical_score(query, suggestion_text)
...
def semantic_search_suggestions(req):
    server = _server()
    ...
    cache_key = _fast_suggestion_cache_key(req)
    ...
    def add_candidate(
        ...
        normalized = server._normalize_text(text)
    ...
    upstream_future = server.recommendation_executor.submit(
        server.ytmusic.get_search_suggestions,
        query,
    )
    artist_future = server.recommendation_executor.submit(
        server._assistant_tool_search_artists_direct,
        query,
        3,
    )
    track_future = server.recommendation_executor.submit(
        server._ytmusic_song_search,
        query,
        4,
    )
    ...
    try:
        upstream_suggestions = upstream_future.result(timeout=0.9)
    except Exception:
    ...
    try:
        direct_artists = artist_future.result(timeout=0.75)
    except Exception:
        direct_artists = []
    ...
    try:
        direct_tracks = track_future.result(timeout=0.85)
    except Exception:
        direct_tracks = []
    ...
    for index, recent_query in enumerate(request_recent_queries):
        ...
        if not recent_query_text:
            continue
        if normalized_query and normalized_query not in server._normalize_text(recent_query_text):
            continue
        add_candidate(
            recent_query_text,
    ...
    for item in candidates.values():
        ...
        lexical_score = _fast_lexical_suggestion_score(query, suggestion_text)
        ranking_score = (
