...
from ..domain.catalog import normalized_album_payload, normalized_track_payload
...
_UNOFFICIAL_MEDIA_RE = re.compile(
    r"\b(tribute|karaoke|soundalike|lounge|juke[- ]?box|fitness mix|"
    r"fitness|workout mix|background music|coffee shop music|relaxing piano|"
    r"cover band|revival|various artists|ultimate rock|nature sounds|"
    r"relaxing guitar|relaxing music|meditation music|sleep music|"
    r"instrumental cover|acoustic cover|chillout compilation)\b",
    re.IGNORECASE,
)
...
def _quality_penalty(item: Dict[str, Any]) -> float:
    ...
    penalty = 0.0
    if _UNOFFICIAL_MEDIA_RE.search(text):
        penalty += 2.8
...
def _source_authority(item: Dict[str, Any]) -> str:
    ...
    if _OFFICIAL_SOURCE_RE.search(text):
        return "official"
    if str(item.get("album_id") or item.get("browseId") or "").strip():
        return "canonical"
    if str(item.get("id") or "").strip() and artist_name(item) and album_name(item):
...
def _fast_artist_neighborhood(server: Any, taste: TasteProfile) -> Dict[str, Any]:
    ...
    for seed_name in _artist_queries(taste)[:4]:
        ...
        for related_artist in related:
            ...
            try:
                tracks.extend(server._assistant_tool_search_tracks(f"{name} songs", 12) or [])
            except Exception:
                continue
